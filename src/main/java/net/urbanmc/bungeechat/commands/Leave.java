...
public class Leave extends Command {
	...
	public Leave() {
		super("bcleave", "bungeechat.leave");
	}
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (args.length == 0) {
			comp = new ComponentBuilder("Please enter a chat to leave.").color(ChatColor.RED).create()[0];
		...
		if (!p.hasPermission("bungeechat.leave." + channel.getName())) {
			comp = new ComponentBuilder("You do not have permission to leave this channel.").color(ChatColor.RED)
					.create()[0];
			p.sendMessage(new BaseComponent[] { prefix, comp });
		...
		if (!channel.isAbsent(p.getUniqueId())) {
			channel.leave(p.getUniqueId());

			comp = new ComponentBuilder("You have left ").color(ChatColor.DARK_GREEN).create()[0];
		} else {
			comp = new ComponentBuilder("You are not in ").color(ChatColor.RED).create()[0];
		}
